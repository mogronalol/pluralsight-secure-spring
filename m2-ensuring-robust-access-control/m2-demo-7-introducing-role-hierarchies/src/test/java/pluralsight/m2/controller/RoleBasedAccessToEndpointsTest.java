...
public class RoleBasedAccessToEndpointsTest {
    ...
    public void adminAccountsPageIsSecuredByRoles(final Roles role, final boolean permitted) throws Exception {
        ...
        if (permitted) {
            ...
            perform
                    ...
                    .andExpect(content().string(role.equals(Roles.CUSTOMER) ? not(containsAdminTransactions) : containsAdminTransactions));
        } else {
            perform
                    .andExpect(status().is(403));
        }
    ...
    @ParameterizedTest
    @ArgumentsSource(RoleBasedArgumentsProvider.class)
    @AllowedRoles({Roles.CUSTOMER_SERVICE_MANAGER, Roles.CUSTOMER_SERVICE, Roles.SENIOR_VICE_PRESIDENT})
