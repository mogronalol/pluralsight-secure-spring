...
public class Utils {
    public static Authentication createTestAuthentication(final Roles role) {
        final UserDetails principal = new User("user", "password", role.getGrantedAuthorities());
        return new UsernamePasswordAuthenticationToken(principal, "password", role.getGrantedAuthorities()) {
            @Override
            public String toString() {
                return String.format("{Role=%s, Authorities=%s}", role.name(), role.getGrantedAuthorities());
            }
        };
    }
